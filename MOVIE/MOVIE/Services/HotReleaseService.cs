...
namespace MOVIE.Services
{
    class HotReleaseService
    {
        ...
        public async Task<string> _getAPIConfig()
        {
            ...
            try
            {
                HttpResponseMessage response = await httpClient.GetAsync("https://api.themoviedb.org/3/configuration?api_key=" + apiKey);
        ...
        public async Task<List<Movie>> _getHotReleaseList()
        {
            httpClient = new HttpClient();

            try
            {
                HttpResponseMessage response = await httpClient.GetAsync("https://api.themoviedb.org/3/movie/now_playing?api_key=" + apiKey);
                response.EnsureSuccessStatusCode();
                string responseBody = await response.Content.ReadAsStringAsync();
                ...
                {
                    response = await httpClient.GetAsync("https://api.themoviedb.org/3/movie/" + id + "?api_key=" + apiKey + "&language=en-US");
                    ...
                    string ParsedDate = ReleaseDate.ToString(pattern);
                    DateTime ConstDate = DateTime.ParseExact(ParsedDate, pattern, CultureInfo.InvariantCulture).Date;
                    ...
                    Movie movie = new Movie(ID, Adult, Buget, HomePage, OriginalTitle, Overview, Popularity, PosterPath, ConstDate, Revenue, Status, TagLine, VoteAverage, VoteCount, genreList);
                    movieList.Add(movie);
                }
                return movieList;
            }
